...
> This document is a verbatim transcription of the original GM Binder compendium, with structure normalized for markdown. Author's spelling and grammar have been preserved.
...
- **Language Training:** Language Training Level 1-4 (English), OR Language Training Level 1-3 (English) plus Level 1 in another language of your choice.
...
In your initial training during the Viper Team endoc you picked up on a few styles of combat. Pick one trait from below. Gain one more trait at Levels 10 and 20. You may take the same style more than once.
...
You have a limited well of stamina. On your turn you can use a bonus action to regain hit points equal to 1d10 + your class level. Once used, you must finish a short or long rest before you can use it again. May only be used if you are at Triage code Orange or better.

## Operator Training

At 2nd level, and again at 4th, 6th, 8th, 10th, 12th, 14th, 16th, 18th and 20th level, you pick 2 Levels in any Training Program you choose. You may not use both levels on the same upgrade with a single use of Operator Training.
...
At 3rd level, choose a Combat Designation: Rifleman, Marksman, Machine Gunner, Demolitionist, Unit Leader, Medic, Dog Handler, or Tech Specialist. The chosen Designation grants features at 3rd, 7th, 10th, 15th, and 18th level.
...
**Rally.** Bonus action. Expend a Leadership Die to pick an ally within 30 ft. They regain Leadership Die + Charisma modifier HP (only if at Triage Yellow or Green). May target multiple at 1 die each.
...
Expend one use of the Trauma Bag and roll a Wisdom (Medicine) check DC 15. On pass, restore 1d10 + Wisdom modifier HP (only on targets at Triage Yellow, Orange, or Red). Increases to 2d10 at 10th, 3d10 at 20th.
...
- **5 Days Diazepam:** After 5 consecutive days, each day's use lets you treat yourself as one Shaken level less than current, but not for healing purposes.
...
### Master Handler (18th)
Gain a second War Dog of your choice with 6 War Dog Training traits. When you command one dog to take an action, the other may take the same (or an equivalent) action.
...
During a long rest, modify your drone with a Technology (Int) DC 15 check. Pass = one upgrade. Fail = no upgrade, retry next rest. Critical fail = drone unusable until next long rest.
...
### Tech Package (7th)
You gain:
- **3× Motion Sensors:** 2-inch, 4 mm thick discs that ping every 6 seconds when detecting movement within 20 ft. Bonus action to activate. Last 24 hours.
- **2× Seeker Grenades:** Sticky grenade requiring no attack so long as it can roll; moves 40 ft and self-detonates after 10 rounds.
...
### MacGyver (15th)
With a Drone Repair Kit, create or maintain basic Tech devices in the field — audio bugs, basic comms, fixing a radio, etc. (GM approval; Technology check, DC by GM). Your E B1-RD gains 1 permanent upgrade.
...
- **Level 1:** Gain Brawler Points equal to 2× your combined levels in Unarmed and Advanced Unarmed Training. Learn 3 Brawler techniques.
...
- **Level 1:** Use a proficient ranged weapon as a blunt weapon (2H = 1d6, 1H = 1d4). With a bayonet, slashing or piercing instead.
...
- **Level 1:** Sneak Attack 2d6 (once per turn) with advantage on attack; alternately, ally within 5 ft of target. Finesse, unarmed, or ranged.
...
*Each level once per skill.*
- **Level 1:** Proficiency in History, Investigation, Nature, Religion, Animal Handling, Insight, Medicine, Perception, Survival, Deception, Intimidation, Performance, or Persuasion.
- **Level 2:** +1 to checks.
...
- **Level 1:** Advantage on melee attacks if an ally is adjacent; advantage on CQC weapons if an ally is at a 90° offset.
...
The B-1 Recon Drone (B1-RD or "bird") is a small man-portable 4-rotor drone weighing ~3.6 kg including its command console. Deployed as an action. When operating, you command the drone via a 2-handed controller/screen. This uses your full attention — you may not move, act, or react while piloting.
...
**Spray:** As an action, choose adjacent targets up to Dex modifier. Apply -5 per target beyond the first. Consumes 1d4+2 rounds per target (roll per attack). On a hit, score one extra hit per 5 whole points your attack exceeded AC (max = rounds fired in the volley).

**Riddle:** As an action, choose volleys up to Dex modifier on a single target. Apply -5 per volley beyond first. Each volley consumes 1d4+2 rounds. Extra-hit rule as above.

**Controlled Burst:** One burst per attack available. -5 per range multiple. Consumes 1d4+2 rounds. Extra-hit rule applies.

## Burst Fire

For weapons with the burst property. One burst per attack. -5 per range multiple. Consumes 3 rounds. Extra-hit rule applies.
...
As an action: 3 rounds, 1 volley of 1d4+2, or 1 burst per target. Target creatures within 10 ft of each other, up to Dex modifier. Suppressed targets have disadvantage on ranged attacks and Wis (Perception) next turn. Cannot use with Manual Action weapons.
