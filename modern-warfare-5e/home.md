---
title: Modern Warfare 5E Compendium
aliases: [Home, Index]
tags: [moc]
---

# Modern Warfare 5E Compendium

The homebrew rules set for realistic modern combat in D&D 5E. Use the index below to jump to a section.

## Character

- [Character Creation](character-creation.md) - attributes and personality traits.
- [Operator Class](operator-class.md) - core class features, training, and progression.

## Combat Designations

- [Combat Designations](combat-designations.md) - overview and links to each designation.

## Training Programs

- [Training Programs](training-programs.md) - overview and links to each program category.

## Equipment

- [Weapons](weapons.md) - firearms, launchers, grenades, and explosives by category.
- [Armor](armor.md) - torso armor, helmets, and plate protection.
- [Clothing](clothing.md) - civilian, military, and tactical apparel, including protective gloves and boots.
- [Gear](gear.md) - standard-issue equipment and the B-1 Recon Drone.

## Rules

- [Rules](rules.md) - weapon properties, modifications, combat actions, conditions, and damage types.
