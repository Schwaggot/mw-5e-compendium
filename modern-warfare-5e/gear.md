---
title: Gear
aliases: [ Equipment, Kit ]
tags: [ moc, gear, equipment ]
---

# Gear

Clothing, packs, field kit, comms, optics, electronics, medical, demolitions, and drones. Item effects are primarily narrative -
each item carries tags that the GM uses to rule on situational advantage, access, or capability. Weight values are
provided so Operators can track encumbrance.

## Gear Tags

| Axis      | Tag            | Effect                                                                              |
|-----------|----------------|-------------------------------------------------------------------------------------|
| Vision    | night-vision   | Treat darkness/dim light as bright light out to device range (typ. 100 ft).         |
| Vision    | thermal        | See heat signatures through smoke, foliage, and light concealment.                  |
| Vision    | magnified      | Advantage on Perception (sight) at long range; disadvantage to spot close movement. |
| Signal    | encrypted      | Comms cannot be intercepted or read by unequipped parties.                          |
| Signal    | long-range     | Effective beyond standard squad comms (regional/sat coverage).                      |
| Signal    | short-range    | Line-of-sight only; small radius (typ. under 1 km).                                 |
| Signal    | jams-comms     | Suppresses enemy radio traffic in a short radius.                                   |
| Detection | detects-motion | Alerts to movement in a defined area.                                               |
| Detection | detects-rf     | Picks up radio or electronic emissions in range.                                    |
| Utility   | breaching      | Can defeat locks, doors, walls, or similar obstacles.                               |
| Utility   | lockpicks      | Enables Sleight of Hand / Thieves' Tools checks vs mechanical locks.                |
| Utility   | repair         | Enables repair checks on a specific class of equipment (drone, weapon, vehicle).    |
| Medical   | stops-bleed    | Stabilizes a bleeding target; interacts with Triage Code.                           |
| Medical   | revives        | Brings an unconscious target back to Triage Code Yellow at GM discretion.           |
| Carry     | covert         | Conceals contents from casual inspection (civilian blend).                          |
| Carry     | high-capacity  | Holds substantially more than a standard pack; incurs weight.                       |

## Mission Kits

All-in-one bundles that answer "what's the op?" - grab a row for the mission type. The sections linked below break each
kit into individual pieces.

| Kit                  | Weight | Role                                   | Contents (summary)                                                             |
|----------------------|--------|----------------------------------------|--------------------------------------------------------------------------------|
| Direct Action        | ~18 kg | Offensive raid / HVT action            | Main ruck + IFAK + NVG + encrypted squad radio + breaching charge + extra mags |
| Reconnaissance       | ~12 kg | Long-range stealth observation         | Day pack + binoculars + spotter scope + radio + GPS + light survival kit       |
| Long-Range Patrol    | ~25 kg | Multi-day overland insertion           | Full ruck + sat radio + NVG + GPS + rations + survival kit + water filter      |
| Hostage Rescue / CQB | ~15 kg | Building clear, breach-and-rescue      | Chest rig + IFAK + NVG + flashbangs + breaching charge + encrypted radio       |
| Protective Detail    | ~6 kg  | Civilian escort / low-profile security | Covert pouch + concealable IFAK + earpiece + short-range radio                 |
| Humanitarian / SERE  | ~10 kg | Non-combat or escape-and-evasion       | Day pack + expanded medical + radio + GPS + survival kit                       |

## Designation Kits

The tool proficiency granted by each [Combat Designation](combat-designations.md) comes bundled as an all-in-one pack.
Add the row matching your designation to your inventory instead of tracking the individual pieces.

| Kit             | Weight  | Designation                                                      | Contents (summary)                                                                                                                    |
|-----------------|---------|------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------|
| LBS Kit         | ~2.5 kg | [Rifleman](designations/rifleman.md)                             | Modular MOLLE chest rig or load-bearing vest with mag, admin, grenade, IFAK, and radio pouches (empty weight).                        |
| T&E Kit         | ~2.0 kg | [Machine Gunner](designations/machine-gunner.md)                 | Traverse and Elevation mechanism for tripod-mounted MGs; dial-and-scale head, clamp bracket, T&E plate.                               |
| Observation Kit | ~2.0 kg | [Marksman](designations/marksman.md)                             | Spotting scope with field tripod, laser rangefinder, wind meter, DOPE book, and range cards.                                          |
| Demolitions Kit | ~3.0 kg | [Demolitions Specialist](designations/demolitions-specialist.md) | Electric and non-electric caps, time fuse, det cord, crimpers, clacker, galvanometer, wire, tape, EOD probes.                         |
| Command Kit     | ~3.0 kg | [Unit Leader](designations/unit-leader.md)                       | Map case with laminated sheets, compass, protractor, range cards, tactical radio, and binoculars.                                     |
| Medic's Kit     | ~2.0 kg | [Combat Medic](designations/combat-medic.md)                     | Stethoscope, BP cuff, pulse oximeter, airways, BVM, decompression needle, suture kit, IV gear, syringes.                              |
| Handler's Kit   | ~3.5 kg | [Dog Handler](designations/dog-handler.md)                       | Working lead, long tracking line, tactical harness and muzzle, bite sleeve, clicker and whistle, e-collar, reward pouch, canine IFAK. |
| B1ARD Kit       | ~2.0 kg | [Tech Specialist](designations/tech-specialist.md)               | Diagnostic unit, spare rotors and optics, micro-tools, firmware loader, and replacement battery pack.                                 |

## Gear Index

- [Clothing](gear/clothing.md) - civilian, military, and tactical apparel, including protective gloves and boots.
- [Packs](gear/packs.md) - rucksacks, assault packs, chest rigs, battle belts, pouches.
- [Field Kit](gear/field-kit.md) - standard-issue items, canteens, sleeping bags, multi-tools, flashlights.
- [Consumables](gear/consumables.md) - rations, drinks, stimulants, vices, and cooking fuel.
- [Comms](gear/comms.md) - walkie-talkies, squad and sat radios, earpieces, crypto keys.
- [Optics](gear/optics.md) - night-vision, thermal, binoculars, monoculars, rangefinders, spotters.
- [Electronics](gear/electronics.md) - jammers, scanners, bugs, sensors, IR strobes, lock-pick sets.
- [Medical](gear/medical.md) - IFAK, trauma bags, tourniquets, hemostatic agents, stims.
- [Demolitions](gear/demolitions.md) - C4, det cord, blasting caps, clackers, breaching charges.
- [Drones](gear/drones.md) - B-1 Recon Drone, Black Hornet, loitering munitions.
