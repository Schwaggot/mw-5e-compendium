---
title: Rules
aliases: [ ]
tags: [ moc, rules ]
---

# Rules

Rules references pulled into one section for quick lookup during play.

## Core Mechanics

- [Attributes](rules/attributes.md) - the six ability scores (STR, DEX, CON, INT, WIS, CHA) and what they cover in this
  universe.
- [Proficiency](rules/proficiency.md) - Proficiency Bonus, what counts as trained, and Expertise.
- [Ability Checks](rules/ability-checks.md) - rolling a check, the DC ladder, contested and passive checks.
- [Saving Throws](rules/saving-throws.md) - what a save is, the six save types, and Save DCs.
- [Advantage and Disadvantage](rules/advantage.md) - 2d20 take higher / take lower, stacking rule, common sources.
- [Movement](rules/movement.md) - speed, difficult terrain, prone and crouched, dash, disengage, climb and jump.
- [Encumbrance](rules/encumbrance.md) - carry capacity by Strength, light / encumbered / heavily encumbered bands.
- [Hit Points](rules/hit-points.md) - HP, Triage Code, going down at 0 HP, death saves, and stabilization.
- [Rest](rules/rest.md) - short and long rests, Hit Dice spending, and feature recovery.

## Combat and Equipment

- [Combat Turn](rules/combat-turn.md) - initiative, surprise, the action / bonus action / reaction economy.
- [Combat Actions](rules/combat-actions.md) - firing modes, aim, opportunity attacks, suppression.
- [Cover](rules/cover.md) - half / three-quarters / full cover, concealment, and how cover is granted or defeated.
- [Vision and Light](rules/vision-and-light.md) - bright / dim / darkness, obscurement, NVG, thermal, smoke.
- [Stealth and Hiding](rules/stealth.md) - the Hide action, when you can hide, and what hidden gets you.
- [Critical Hits](rules/critical-hits.md) - per-weapon Crit range, double dice, head-shot crits, and auto-crit features.
- [Conditions](rules/conditions.md) - standard 5E conditions plus homebrew additions (Bleed, Crouched, Shaken, etc.).
- [Damage Types](rules/damage-types.md) - damage types, damage reduction, stress damage, subdual, and target zones.
- [Weapon Properties](rules/weapon-properties.md) - property keywords used by weapons.
- [Weapon Modifications](rules/weapon-modifications.md) - mod options and per-category availability.
- [Ammunition](rules/ammunition.md) - magazine tracking, weights, starting loadout, and resupply.
